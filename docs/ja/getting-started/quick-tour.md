...
ダウンロードは、進捗ダイアログの Cancel で中断できます。

手元のファイルを開く場合は **File &gt; Open File...** を使います。
読み込み時に Renderer の種類を選ぶ画面が出ます (下記参照)。

## 3. 表示を整える

読み込んだ構造は、**Renderer** (表示方法) を通じて**分子ビュー** (中央の 3D 表示領域) に描かれます。
1 つの Object に複数の Renderer を付けて、主鎖はリボン・活性部位は棒球モデル、といった重ね方ができます。

### Renderer を追加する

左サイドパネルの **Explorer &gt; Scene** ツリーで Object を右クリックし、
