...
サイドパネルの **Color** パネルで、Renderer 単位に設定します。

このページは各方式のパラメータを網羅するリファレンスです。
...
| Multi-gradient <span class="badge-wip">開発中</span> | 複数の色を経由するグラデーション。**現在のバージョンでは選択できません** | — |
