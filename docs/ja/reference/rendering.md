...
エネルギーを、大域照明の照り返しにどれだけ移すかを段階で選びます。斜め上からのキーライトの
強さは、どの段でも変わりません。
...
| 2 | |
| 3 | |
| 4 (max GI) | ヘッドライトを完全に消し、キーライトと照り返しだけで陰影を付けます (**既定**) |
...
| 項目 | 既定値 | 範囲 | 説明 |
|---|---|---|---|
| Light intensity | 1.3 | 0〜3 | 光源全体の強さ |
| Flash fraction | 0 | 0〜1 | 直接光のうち、視点方向からのヘッドライトに割り当てる割合。残りが斜め上からのキーライトになります。小さいほど陰影がはっきりします |
...
1.55 / 0.6 です。そこから Global Illumination に戻すと、**GI lighting は既定の段 (4) で
入り直します**。
...
| Contact edges | ON | | エッジ線の設定が異なる Renderer が交わる線を描きます (下記) |
...
すべての Renderer が同じエッジ設定 (既定のままなど) のシーンでは、交線は描かれません。
分子ビューにはこの交線は描かれません。この設定を OFF にすると交線が描かれなくなり、
Edge type が Silhouette の Renderer では、他の Renderer と接する部分で輪郭が途切れます。

### Transparency

半透明の Renderer が重なったときの合成方法です。Umbreon と Umbreon (NPR) の両方にあります
(POV-Ray にはありません)。既定では折りたたまれています。

| 項目 | 既定値 | 説明 |
|---|---|---|
| Per-pixel transparency | ON | 半透明の合成を画素ごとに行います |

半透明の Renderer は、それぞれを不透明として描いてから合成されます。ON のときは、
その画素を実際に覆っている層だけで重みを決めるため、背景の重みが負になりません。
OFF は従来の層単位の合成で、不透明度の和が 1 を超える半透明が重なった画素では背景の重みが
負になり、**そこに重なった暗いものが明るく反転**します (半透明の表面越しの黒いエッジ線が
白く出る、など)。

半透明が 1 枚しか掛かっていない画素は、ON でも OFF でも同じ絵になります。差が出るのは
重なった部分だけです。レンダリングにかかる時間はどちらでも変わりません。
blendpng や POV-Ray と同じ結果が必要なときだけ OFF にしてください。
...
| Ambient fraction | 0.6 | 0〜1 | 光全体のうち、大域照明の照り返しとして集める割合。**GI lighting** の段が書き換えます |
...
:   Edge type の **Silhouette** は、Umbreon では**外周の輪郭だけ**を描くモードになります。
    エッジ線の設定 (種類・太さ・色) が同じ Renderer はひとまとまりとして 1 つの外周になり、
    重なり合う部分には線が入りません。
