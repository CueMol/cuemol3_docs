...
レイトレーシング時に、描画された物体の輪郭に線 (エッジ線) を描き加える設定です。
...
!!! note "分子ビューには反映されません"
    エッジラインの設定は、レンダリングウィンドウでのレイトレーシング出力
    (Umbreon / POV-Ray) にのみ影響します。分子ビュー (リアルタイム表示) の見た目は
    変化しません。

<!-- TODO(verify): tritium の OpenGL 分子ビューでエッジラインが描画されないことの再確認 -->
