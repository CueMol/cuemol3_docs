...
タンパク質をカートゥーンで表示し、リガンドを棒球モデルで重ね、色と輪郭線を整えて
...
<!-- TODO(screenshot): 完成イメージ。3UG9 のカートゥーン (鎖ごとに塗り分け) + レチナールの棒球モデル、白背景 + エッジライン -->
...
タンパク質部分が、二次構造を反映したカートゥーン表示で描かれます
...
カートゥーンは主鎖の走行だけを描くため、リガンドは表示されません。
...
4. **Create** を押すと、カートゥーンに重ねてレチナールが棒球モデルで描かれます
