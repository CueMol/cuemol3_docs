...
- **起動時のちらつきが無くなりました。** ウィンドウは中身が描き終わってから表示され、
  最初の一瞬だけ出ていた巨大な中心マークも無くなりました。
...
- **共通ライブラリの点検で 132 件の不具合が修正されました。** CueMol2 と共通の部分で、
  原子名の照合、二次構造の判定、PDB の MODEL の扱い、Undo / Redo、壊れたファイルの
  読み飛ばしなどが含まれます。

!!! warning "原子が読み飛ばされた `.gro` ファイルにはトラジェクトリを重ねられません"
    同じ残基に同じ原子名の原子が現れた場合、その原子は警告を出して読み飛ばします。
    この場合、読み込んだ原子の数がファイルと合わなくなるため、
    `.xtc` / `.trr` / `.dcd` のトラジェクトリを重ねられません。
    振り直された残基番号は、元の番号に 100000 を足したものになります。
