...
4. 更新履歴 (`docs/ja/releases.md`) にエントリを足す (書式と載せる基準は下記
   「更新履歴 (releases.md) の運用」を参照)。
5. 下記の同期記録を更新する。
6. **同期レポート**を書く (PR の説明、または作業報告)。事実起因の書き換えは 1 件ずつ
   「ページ / 旧記述の要旨 → 新しい事実 / 根拠 (cuemol2 側の変更)」の形式で列挙する。
   判断を保留した箇所も同じ形式で「要判断」として挙げる。
   ユーザーはこの一覧を軸に diff をレビューする。
7. 変更は**ブランチで行い、ユーザーが diff を確認してから** main へ。push はユーザーの確認後
   (main への push は即デプロイされる。PR には check.yml の strict build が走る)。

## 更新履歴 (releases.md) の運用

リリースが週 1 本のペースで続くため、放っておくと 1 ページが際限なく伸びる。
次の 2 つで抑える。

### 載せる基準 (何を書かないか)

**書くのは、ユーザーの操作や見え方が変わるものだけ。** 次は書かない
(マニュアル本体に書かないのと同じ基準を、更新履歴にも適用する)。

- **不具合修正**。「本来こう動くはずのものが動くようになった」類は、直った先の
  動作をマニュアルが既に書いているので、更新履歴にも載せない。
- **内部的な改善・リファクタリング**。メモリ使用量の削減、クラッシュの回避、
  UI の文字サイズ、入力欄の細かい挙動など、読み手が何かする余地が無いもの。

判断がつかないものは書かずに同期レポートへ挙げ、ユーザーの判断を仰ぐ。
1 リリースあたり 3〜5 節・60〜90 行が目安。

### ページを分ける

`docs/ja/releases.md` には**直近 5 リリース**だけを置く。6 本目を足すときに、
いちばん古いエントリを `docs/ja/releases-archive.md` (過去のリリース) の先頭へ
**そのまま移す** (本文は書き換えない)。あわせて `releases.md` の版番号を 3 か所直す。

- 冒頭の「直近の 5 リリースを載せています」に続く、アーカイブへの案内
- 末尾の「<version> 以前のリリースは…」
- `## 関連項目` のアーカイブへのリンクの説明

`releases-archive.md` は nav に登録済みなので、ファイルを増やす必要はない。

アーカイブ側はさらに伸びるが、こちらは引くためのページなので許容する。
